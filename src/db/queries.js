...
function setParticipantTime(shiftId, userId, field, time) {
  // field: 'started_at' or 'ended_at', time: 'HH:MM' or null
  getDb().prepare(
    `UPDATE shift_participants SET ${field} = ? WHERE shift_id = ? AND user_id = ?`
  ).run(time, shiftId, userId);
}
