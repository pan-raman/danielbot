...
// Exported so /testreminder can call the same logic
  ...
  for (const shift of shifts) {
    ...
    if (isWithin(shift.start_time, 5)) {
      for (const user of participants) {
        ...
        try { await sendStartNotification(bot, user.id, shift); markNotified(shift.id, user.id, 'notified_start'); } catch {}
      }
    }
    ...
    if (isWithin(shift.end_time, 5)) {
      for (const user of participants) {
        ...
        try { await sendEndNotification(bot, user.id, shift); markNotified(shift.id, user.id, 'notified_end'); } catch {}
      }
    }
...
function setupReminders(bot) {
  cron.schedule('* * * * *', () => runReminderCheck(bot));
}
